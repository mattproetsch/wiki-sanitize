...
var wikisanitize = (function() {
	...
	// TODO: Replace this function with some fancy ML! #onlypartlyjoking
	var extractInterestingSummaries = function(editSummaries) {
		...
		for (var i = 0; i < editSummaries.length; i++) {
			for (var j = 0; j < interestingPhrases.length; j++) {
				if (interestingPhrases[j].re.test(editSummaries[i].sum)) {
					retn.push(editSummaries[i]);
					// No need to test against any more phrases
					break;
				}
			}
		}
...
var getMoney = function(title, wikiWorker) {
	...
	var processEditors = function() {
		...
		for (var i = 0; i < Math.min(5, editors.length); i++) {
			let userPageWorker = wikisanitize.getUserPageWorker(editors[i], function(snippets, err) {
				if (err === null) {
					infoPanel.port.emit("appendInterestingUserPageSnippets", snippets);
				}
				else {
				...
				alertWorker.port.emit("User worker " + i.toString() + " finshed - flagging");
				flagWorkerForDeletion("userPageWorker" + i.toString());
				cleanupWorkers();
			});
			workers.push({
				worker: userPageWorker,
				name: "userPageWorker" + i.toString(),
				flag: false
			});
		}
	}
	...
	var editorListPageWorker = wikisanitize.getEditorsWorker(title, function(editorList, err) {
		if (err === null) {
			editors = editorList;
	...
	var editSummariesPageWorker = wikisanitize.getEditSummariesWorker(title, function(summariesList, err) {
		if (err === null) {
